...
> **Good to know**: `'use cache'` is stored in-memory by default, so entries don't persist across process restarts or deploys. For data that needs to survive a redeploy, use [`unstable_cache`](/docs/app/api-reference/functions/unstable_cache) for non-`fetch` functions or the [`fetch`](/docs/app/api-reference/functions/fetch) cache.
