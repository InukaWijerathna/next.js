      ...
      case 'request':
        ...
        if (workUnitStore.asyncApiPromises && stagedRendering) {
          ...
          if (
            ...
            const stage = getStagesByDataKind(stagedRendering).staticLinkData
            return stagedRendering.delayUntilStage(
              isRuntimePrefetchable ? stage.early : stage.late,
              'params',
              underlyingParams
            )
          }
  ...
  switch (prerenderStore.type) {
    case 'prerender': {
      ...
      if (
        ...
        return stagedRendering.delayUntilStage(
          // static prerenders don't distinguish early/late, this is just using
          // the same function for consistency.
          getStagesByDataKind(stagedRendering).staticLinkData.late,
          'params',
...
function createRuntimePrerenderParams(
  ...
  const paramsStage = getStagesByDataKind(stagedRendering).runtimeLinkData
  const stage = isRuntimePrefetchable ? paramsStage.early : paramsStage.late
