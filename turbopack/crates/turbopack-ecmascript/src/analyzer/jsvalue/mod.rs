...
pub enum JsValue<'a> {
    ...
    Function(u32, u32, bumpalo::boxed::Box<'a, JsValue<'a>>),
    ...
    SuperCall(u32, bumpalo::boxed::Box<'a, [JsValue<'a>]>),
    ...
    Promise(u32, bumpalo::boxed::Box<'a, JsValue<'a>>),
    /// An await call (potentially) unwrapping a promise.
    /// `(total_node_count, value)`
    Awaited(u32, bumpalo::boxed::Box<'a, JsValue<'a>>),

    /// A for-of loop
    ///
    /// `(total_node_count, iterable)`
    Iterated(u32, bumpalo::boxed::Box<'a, JsValue<'a>>),

    /// A `typeof` expression.
    ///
    /// `(total_node_count, operand)`
    TypeOf(u32, bumpalo::boxed::Box<'a, JsValue<'a>>),
...
impl<'a> JsValue<'a> {
    ...
    pub fn tenary(arena: &'a Bump, test: JsValue<'a>, cons: JsValue<'a>, alt: JsValue<'a>) -> Self {
        Self::Tenary(
            ...
            bumpalo::boxed::Box::new_in(test, arena),
            bumpalo::boxed::Box::new_in(cons, arena),
            bumpalo::boxed::Box::new_in(alt, arena),
        )
    }

    pub fn iterated(arena: &'a Bump, iterable: JsValue<'a>) -> Self {
        Self::Iterated(
            1 + iterable.total_nodes(),
            bumpalo::boxed::Box::new_in(iterable, arena),
        )
    }
    ...
    pub fn equal(arena: &'a Bump, a: JsValue<'a>, b: JsValue<'a>) -> Self {
        Self::Binary(
            ...
            bumpalo::boxed::Box::new_in(a, arena),
            BinaryOperator::Equal,
            bumpalo::boxed::Box::new_in(b, arena),
        )
    }
    ...
    pub fn logical_not(arena: &'a Bump, inner: JsValue<'a>) -> Self {
        Self::Not(
            1 + inner.total_nodes(),
            bumpalo::boxed::Box::new_in(inner, arena),
        )
    }

    pub fn type_of(arena: &'a Bump, operand: JsValue<'a>) -> Self {
        Self::TypeOf(
            1 + operand.total_nodes(),
            bumpalo::boxed::Box::new_in(operand, arena),
        )
    }
    ...
    pub fn function(
        ...
        Self::Function(
            ...
            func_ident,
            bumpalo::boxed::Box::new_in(return_value, arena),
        )
    }
    ...
    pub fn super_call(args: bumpalo::boxed::Box<'a, [JsValue<'a>]>) -> Self {
    ...
    pub fn member(arena: &'a Bump, o: JsValue<'a>, p: JsValue<'a>) -> Self {
        Self::Member(
            ...
            bumpalo::boxed::Box::new_in(o, arena),
            bumpalo::boxed::Box::new_in(p, arena),
        )
    }
...
// Unknown management
impl<'a> JsValue<'a> {
    /// Move the value out of an arena-allocated box, leaving a default ([`JsValue::unknown_empty`])
    /// in the box's slot. Needed because `bumpalo::boxed::Box`, unlike `std::Box`, does not support
    /// moving its contents out via `*box`.
    pub fn unbox(mut boxed: bumpalo::boxed::Box<'a, JsValue<'a>>) -> JsValue<'a> {
        take(&mut *boxed)
    }
...
impl<'a> JsValue<'a> {
    ...
    pub fn clone_in(&self, arena: &'a Bump) -> JsValue<'a> {
        match self {
            ...
            JsValue::Function(c, id, r) => JsValue::Function(
                *c,
                *id,
                bumpalo::boxed::Box::new_in(r.clone_in(arena), arena),
            ),
            ...
            JsValue::Not(c, v) => {
                JsValue::Not(*c, bumpalo::boxed::Box::new_in(v.clone_in(arena), arena))
            }
            ...
            JsValue::Binary(c, a, op, b) => JsValue::Binary(
                *c,
                bumpalo::boxed::Box::new_in(a.clone_in(arena), arena),
                *op,
                bumpalo::boxed::Box::new_in(b.clone_in(arena), arena),
            ),
            ...
            JsValue::Member(c, o, p) => JsValue::Member(
                *c,
                bumpalo::boxed::Box::new_in(o.clone_in(arena), arena),
                bumpalo::boxed::Box::new_in(p.clone_in(arena), arena),
            ),
            JsValue::Tenary(c, test, cons, alt) => JsValue::Tenary(
                *c,
                bumpalo::boxed::Box::new_in(test.clone_in(arena), arena),
                bumpalo::boxed::Box::new_in(cons.clone_in(arena), arena),
                bumpalo::boxed::Box::new_in(alt.clone_in(arena), arena),
            ),
            JsValue::Promise(c, v) => {
                JsValue::Promise(*c, bumpalo::boxed::Box::new_in(v.clone_in(arena), arena))
            }
            JsValue::Awaited(c, v) => {
                JsValue::Awaited(*c, bumpalo::boxed::Box::new_in(v.clone_in(arena), arena))
            }
            JsValue::Iterated(c, v) => {
                JsValue::Iterated(*c, bumpalo::boxed::Box::new_in(v.clone_in(arena), arena))
            }
            JsValue::TypeOf(c, v) => {
                JsValue::TypeOf(*c, bumpalo::boxed::Box::new_in(v.clone_in(arena), arena))
            }
