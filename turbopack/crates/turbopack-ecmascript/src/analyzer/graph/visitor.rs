...
enum EarlyReturn<'a> {
    ...
    Conditional {
        ...
        condition: bumpalo::boxed::Box<'a, JsValue<'a>>,
...
pub(super) struct Analyzer<'a, 'e> {
    pub(super) arena: &'a Bump,

    pub(super) analyze_mode: AnalyzeMode,

    pub(super) data: VarGraph<'a>,
    pub(super) state: analyzer_state::AnalyzerState<'a>,

    pub(super) effects: Vec<Effect<'a>>,
    ...
    pub(super) hoisted_effects: Vec<Effect<'a>>,
    ...
    pub(super) eval_context: &'e EvalContext,
}
...
impl<'a, 'e> Analyzer<'a, 'e> {
        ...
        match callee {
            ...
            Callee::Expr(box expr) => {
                if let Expr::Member(MemberExpr { obj, prop, .. }) = unparen(expr) {
                    let obj_value = bumpalo::boxed::Box::new_in(
                        self.eval_context.eval(self.arena, obj),
                        self.arena,
                    );
                    let prop_value = match prop {
                        // TODO avoid clone
                        MemberProp::Ident(i) => {
                            bumpalo::boxed::Box::new_in(i.sym.clone().into(), self.arena)
                        }
                        MemberProp::PrivateName(_) => bumpalo::boxed::Box::new_in(
                    ...
                    let fn_value = bumpalo::boxed::Box::new_in(
                        self.eval_context.eval(self.arena, expr),
                        self.arena,
                    );
    ...
    fn check_member_expr_for_effects<'ast: 'r, 'r>(
        ...
        let prop_value = match &member_expr.prop {
            // TODO avoid clone
            MemberProp::Ident(i) => bumpalo::boxed::Box::new_in(i.sym.clone().into(), self.arena),
            MemberProp::PrivateName(_) => {
                return;
            }
            MemberProp::Computed(ComputedPropName { expr, .. }) => {
                bumpalo::boxed::Box::new_in(self.eval_context.eval(self.arena, expr), self.arena)
            }
        };
...
impl<'a, 'e> VisitAstPath for Analyzer<'a, 'e> {
    ...
    fn visit_unary_expr<'ast: 'r, 'r>(
        ...
        if n.op == UnaryOp::TypeOf && self.analyze_mode.is_code_gen() {
            let arg_value =
                bumpalo::boxed::Box::new_in(self.eval_context.eval(self.arena, &n.arg), self.arena);
    ...
    fn visit_labeled_stmt<'ast: 'r, 'r>(
        ...
        prev_effects.push(Effect::Conditional {
            condition: bumpalo::boxed::Box::new_in(
...
impl<'a, 'e> Analyzer<'a, 'e> {
    fn add_conditional_if_effect_with_early_return(
        ...
        let condition =
            bumpalo::boxed::Box::new_in(self.eval_context.eval(self.arena, test), self.arena);
